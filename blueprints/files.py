...
def get_user_home_dir_abs(username):
    ...
    # 优先使用 USERS_DB 中为用户配置的 home_dir (如果它是相对于 base_dir 的子目录名)
    # 或者，如果 USERS_DB 没有特定配置，则默认为 USER_FILES_BASE_DIR/username
    # 这里的逻辑简化为：用户的主目录总是 USER_FILES_BASE_DIR/username
    # 如果 config.py 中的 USERS_DB[username]['home_dir'] 意图是 *覆盖* 这个默认行为，
    # 并且它应该是一个相对于 USER_FILES_BASE_DIR 的路径，或者一个绝对路径，
    # 那么这里的逻辑需要调整以匹配 config.py 中的设计意图。
    # 目前，为了与 get_user_files_root 的原始简单意图对齐，我们假设 home_dir 就是 base_dir/username
    
    # 假设用户的家目录就是 USER_FILES_BASE_DIR 下以其用户名命名的文件夹
    # 这与许多系统（如Nextcloud的默认用户数据目录结构）一致
    # current_app.logger.debug(f"Determining home for '{username}': Base='{user_files_base}', UserSubDir='{username}'")
    return os.path.abspath(os.path.join(user_files_base, username))


# get_validated_absolute_path (之前可能是 get_absolute_path)
# 确保路径在用户主目录内，并处理相对路径
def get_validated_absolute_path(user_home_directory_abs, relative_subpath=""):
    if not user_home_directory_abs:
        current_app.logger.error(f"Cannot get absolute path: User home is None.")
        return None
    
    # 标准化并清理 relative_subpath
    if relative_subpath:
        # 移除开头的斜杠，os.path.join 会正确处理
        # normpath 会处理 '..' 和 '.'
        normalized_relative_subpath = os.path.normpath(relative_subpath.lstrip('/\\'))
        
        # 防止相对路径试图跳出当前目录的 '..' (尽管 normpath 会处理，但显式检查更安全)
        if normalized_relative_subpath.startswith('..'+os.sep) or normalized_relative_subpath == '..':
            current_app.logger.warning(f"Potentially unsafe relative subpath '{relative_subpath}' resulted in '{normalized_relative_subpath}'. Denying.")
            return None 

        # 组合路径
        full_target_path_abs = os.path.join(user_home_directory_abs, normalized_relative_subpath)
    else:
        full_target_path_abs = user_home_directory_abs

    # 再次绝对化和标准化，以确保最终路径格式一致
    resolved_abs_path = os.path.abspath(full_target_path_abs)
    user_home_abs_norm = os.path.abspath(user_home_directory_abs) # 确保比较基准也是标准化的

    # 最关键的安全检查：确保解析后的路径仍然在用户的主目录或其子目录中
    if not resolved_abs_path.startswith(user_home_abs_norm):
        current_app.logger.error(
            f"Path traversal attempt or invalid path construction: "
            f"UserHome='{user_home_abs_norm}', RelSubpath='{relative_subpath}', "
            f"ResolvedPath='{resolved_abs_path}'. Path is outside user's home."
        )
        return None
...
def redirect_to_current_path(current_relative_path_for_redirect):
    """统一重定向回当前路径的辅助函数"""
    if current_relative_path_for_redirect:
        return redirect(url_for('.list_files_with_path', subpath=current_relative_path_for_redirect))
    else:
...
def _load_hidden_files_db():
    ...
    if path and os.path.exists(path):
        try:
            with open(path, 'r', encoding='utf-8') as f: return json.load(f) # Added encoding
...
def load_shares_db():
    ...
    if not os.path.exists(db_path):
        ...
        try:
            with open(db_path, 'w', encoding='utf-8') as f: json.dump({}, f) # Added encoding
            return {}
        except IOError as e:
    ...
    try:
        with open(db_path, 'r', encoding='utf-8') as f: # Added encoding
...
def save_shares_db(shared_items):
    ...
    try:
        ...
        with open(db_path, 'w', encoding='utf-8') as f: # Added encoding
            json.dump(shared_items, f, indent=4, ensure_ascii=False) # ensure_ascii=False for Unicode
...
def list_files_with_path(subpath):
    ...
    user_home_abs = get_user_home_dir_abs(current_user_username) # Get user's home first
    if not user_home_abs:
        flash("无法确定用户主目录。", "danger")
        return redirect(url_for('auth.login')) # Or some other appropriate error/redirect

    current_path_abs = get_validated_absolute_path(user_home_abs, subpath)
    ...
    if current_path_relative_to_home is None: 
        flash("无法确定当前相对路径。", "danger") 
        return redirect(url_for('.list_files_root'))

    items_data = []
    hidden_files_db_global = _load_hidden_files_db() 
    shares_db = load_shares_db() 
    user_own_shares_info = shares_db.get(current_user_username, {})
    ...
    try:
        for item_name in sorted(os.listdir(current_path_abs), key=lambda s: s.lower()):
            ...
            if item_rel_to_current_home is None: 
            ...
            item_data = {
                'name': item_name,
                'path_for_url': item_rel_to_current_home, 
                'is_dir': is_dir,
                ...
                'is_shared_by_owner': is_shared_by_me, 
                'shared_with_users': shared_with_whom_list, 
                'item_path_relative_to_owner_home_for_sharing': item_rel_to_current_home,
    ...
    shareable_users = [uname for uname in all_users_db if uname != current_user_username]

    parent_dir_rel = None
    if current_path_relative_to_home: 
        parent_dir_rel = os.path.dirname(current_path_relative_to_home)
        if parent_dir_rel == ".": parent_dir_rel = "" 
        parent_dir_rel = parent_dir_rel.replace("\\", "/")

    return render_template(
        'files/home.html',
        files=items_data,  
        current_path_relative=current_path_relative_to_home, 
        parent_dir_relative_to_home=parent_dir_rel, 
        users_for_sharing=shareable_users, 
        is_admin_tpl=is_admin_template_context 
    )
...
@files_bp.route('/upload', methods=['POST'], endpoint='upload_file_route')
@login_required
def upload_file_route():
    current_path_relative = request.form.get('current_path_for_action', '')
    # Corrected call
    user_files_root = get_user_home_dir_abs(g.user['username']) 
    if not user_files_root:
        flash("无法确定用户文件目录。", "danger")
        return redirect(url_for('files.list_files_root'))

    current_dir_abs = get_validated_absolute_path(user_files_root, current_path_relative)

    if not current_dir_abs:
        flash("无效的上传路径。", "danger")
        return redirect(url_for('files.list_files_root'))

    if 'file_to_upload' not in request.files:
        flash("没有文件被选择上传。", "warning")
        return redirect_to_current_path(current_path_relative)

    file = request.files['file_to_upload'] 
    if file.filename == '':
        flash("没有选择文件或文件名为空。", "warning")
        return redirect_to_current_path(current_path_relative)

    if file:
        original_filename = file.filename.strip() 
        filename_to_save = secure_filename(original_filename) # Option A
        current_app.logger.debug(f"File upload: original='{original_filename}', secured='{filename_to_save}'")

        if not filename_to_save: 
            flash(f"处理后的文件名为空或无效 (原始文件名: '{original_filename}')。", "danger")
            return redirect_to_current_path(current_path_relative)

        target_path = os.path.join(current_dir_abs, filename_to_save)

        if os.path.exists(target_path):
            name, ext = os.path.splitext(filename_to_save)
            counter = 1
            while os.path.exists(target_path):
                new_filename = f"{name}_{counter}{ext}"
                target_path = os.path.join(current_dir_abs, new_filename)
                counter += 1
            filename_to_save = new_filename 
            flash(f"文件已存在，已重命名为 '{filename_to_save}' 并保存。", "info")

        try:
            file.save(target_path)
            flash(f"文件 '{filename_to_save}' 上传成功。", "success")
            current_app.logger.info(f"File '{target_path}' uploaded by user '{g.user['username']}'.")
        except Exception as e:
            flash(f"上传文件 '{original_filename}' (保存为 '{filename_to_save}') 失败: {e}", "danger")
            current_app.logger.error(f"Error uploading file '{target_path}' for user '{g.user['username']}': {e}", exc_info=True)
    
    return redirect_to_current_path(current_path_relative)


@files_bp.route('/create_folder', methods=['POST'], endpoint='create_folder_route')
@login_required
def create_folder_route():
    ...
    if not current_dir_abs: 
        flash("无效的当前路径。", "danger")
        return redirect(url_for('files.list_files_root'))

    # Add this debug line
    current_app.logger.debug(f"Raw folder_name from form: '{request.form.get('folder_name')}'")
    folder_name_raw = request.form.get('folder_name', '').strip()
    current_app.logger.debug(f"Attempting to create folder. Raw name (stripped): '{folder_name_raw}', In relative path: '{current_path_relative}'")
    ...
    if ".." in folder_name_raw or "/" in folder_name_raw or "\\" in folder_name_raw:
        flash("文件夹名称包含无效字符或路径序列 (.., /, \\)。", "danger")
        current_app.logger.warning(f"Invalid folder name attempt: '{folder_name_raw}' by user '{g.user['username']}'")
        return redirect_to_current_path(current_path_relative)
    
    if folder_name_raw == "." or folder_name_raw == "..":
         flash("文件夹名称不能是 '.' 或 '..'", "danger")
         current_app.logger.warning(f"Invalid folder name attempt (dot or dotdot): '{folder_name_raw}' by user '{g.user['username']}'")
         return redirect_to_current_path(current_path_relative)

    folder_name_to_create = folder_name_raw

    new_folder_path_abs = os.path.join(current_dir_abs, folder_name_to_create)

    if os.path.exists(new_folder_path_abs):
        flash(f"文件夹 '{folder_name_to_create}' 已存在。", "warning")
    else:
        try:
            os.makedirs(new_folder_path_abs)
            flash(f"文件夹 '{folder_name_to_create}' 创建成功。", "success")
            current_app.logger.info(f"Folder '{new_folder_path_abs}' created by user '{g.user['username']}'.")
        except OSError as e:
            flash(f"创建文件夹 '{folder_name_to_create}' 失败: {e}", "danger")
            current_app.logger.error(f"Error creating folder '{new_folder_path_abs}' for user '{g.user['username']}': {e}", exc_info=True)
            current_app.logger.debug(f"Raw folder_name from form: '{request.form.get('folder_name')}'")
            folder_name_raw = request.form.get('folder_name', '').strip()
            current_app.logger.debug(f"Attempting to create folder. Raw name (stripped): '{folder_name_raw}', In relative path: '{current_path_relative}'")
    return redirect_to_current_path(current_path_relative)
...
@files_bp.route('/delete-item', methods=['POST']) 
@login_required
def delete_item_route():
    ...
    if item_path_relative_to_home.startswith('shared/'): 
    ...
    item_to_delete_abs = get_validated_absolute_path(user_home_abs, item_path_relative_to_home)

    if not item_to_delete_abs or not os.path.exists(item_to_delete_abs):
        flash("要删除的项目不存在。", "danger")
        return redirect(url_for('.list_files_with_path', subpath=current_view_path_relative))

    if item_to_delete_abs == user_home_abs: 
    ...
    try:
        ...
        if not is_dir:
            ...
            if current_user_username in shares_db and item_path_relative_to_home in shares_db[current_user_username]:
                ...
                if not shares_db[current_user_username]: 
...
def _resolve_file_path_for_action(path_from_url, requesting_user_username):
    ...
    if path_from_url.startswith('shared/'):
        ...
        if len(parts) == 3 and parts[0] == 'shared':
            ...
            if requesting_user_username == shared_item_owner_username: 
            ...
            owner_home_abs = get_user_home_dir_abs(shared_item_owner_username)
            if not owner_home_abs:
                current_app.logger.error(f"Cannot resolve shared item: Owner '{shared_item_owner_username}' home directory not found.")
                return None, "无法解析共享文件所有者路径。", None
            file_abs = get_validated_absolute_path(owner_home_abs, original_file_path_rel_to_owner_home)
        ...
        user_home_abs = get_user_home_dir_abs(requesting_user_username)
        if not user_home_abs:
            current_app.logger.error(f"Cannot resolve own item: User '{requesting_user_username}' home directory not found.")
            return None, "无法解析用户路径。", None
        file_abs = get_validated_absolute_path(user_home_abs, path_from_url)
        ...
        filename_for_action = os.path.basename(file_abs) # Use basename of absolute path to be safe
        current_app.logger.debug(f"  Own item RESOLVED: AbsPath='{file_abs}', DisplayName='{filename_for_action}'")
        return file_abs, None, filename_for_action


@files_bp.route('/download/<path:path_from_url>') 
@login_required
def download_file_route(path_from_url): 
    current_user_username = g.user['username']
...
@login_required
def preview_text_file_route(path_from_url): 
    current_user_username = g.user['username']
...
@login_required
def preview_image_file_route(path_from_url): 
    current_user_username = g.user['username']
    ...
    if not is_file_previewable_as_image(filename_for_preview): 
...
def share_file_action(): 
    ...
    share_with_user = request.form.get('share_with_selected_username') 
    ...
    owner_home_abs = get_user_home_dir_abs(owner_username)
    if not owner_home_abs:
        flash("无法确定所有者主目录。", "danger")
        return redirect(url_for('.list_files_with_path', subpath=owner_current_view_path))
    file_to_share_abs = get_validated_absolute_path(owner_home_abs, file_path_rel_to_owner_home)
    ...
    if not current_file_share_info:
        current_file_share_info = {
            'shared_with': [], 
            'display_name': os.path.basename(file_key_for_db), 
...
@files_bp.route('/share-actions/unshare-file', methods=['POST']) 
@login_required
def unshare_file_action():
    owner_username = g.user['username'] 
    file_path_rel_to_owner_home = request.form.get('file_path_to_unshare_rel_to_owner_home')
    unshare_for_user = request.form.get('username_to_unshare_from') 
    ...
    if unshare_for_user not in file_share_info.get('shared_with', []):
        ...
        return jsonify_status("info", f"用户 '{unshare_for_user}' 未曾被分享此文件。", 200) 
    ...
    for owner_username, owner_files_shared_out in shares_db.items():
        for item_path_rel_to_owner_home, share_info in owner_files_shared_out.items():
            if requesting_user_username in share_info.get('shared_with', []):
                ...
                owner_home_abs = get_user_home_dir_abs(owner_username) # Get owner's home
                original_file_abs_path = None
                if owner_home_abs:
                    original_file_abs_path = get_validated_absolute_path(owner_home_abs, item_path_rel_to_owner_home)
                ...
                shared_on_display = '-'
                if shared_on_str:
                    try: shared_on_display = datetime.fromisoformat(shared_on_str.replace("Z", "+00:00")).strftime('%Y-%m-%d %H:%M') 
                    except ValueError: current_app.logger.warning(f"Could not parse shared_on date: {shared_on_str}")

                item_entry = {
                    'name': display_name, 
                    'path_for_url': shared_item_path_for_url, 
                    ...
                    'is_shared_item': True, 
                    'file_exists_for_recipient': file_exists 
                }
